...
**After this guide:** put secrets in AWS and redeploy → [observability-deployment.md](./deployment.md).
...
If API SSL fails, Cloudflare `cloud.api` must be **DNS only** (grey cloud). See the [OVH deployment runbook](../../deployment/infrastructure/ovh-microsandbox-runbook.md).
...
3. Save the token — you will store it as `BETTERSTACK_API_TOKEN` in [observability-deployment.md](./deployment.md).
...
**Next:** [observability-deployment.md](./deployment.md) — write these into AWS SSM and redeploy the API.
...
- [observability-deployment.md](./deployment.md) — AWS SSM + Vercel + redeploy
