# Pipeline 17 — GitHub → MySQL

**Streams:** 12 | **Destination:** MySQL
...
CREATE TABLE analytics.gh_commits (
    ...
    author_name VARCHAR(255), author_email VARCHAR(255), committed_at DATETIME
) ENGINE=InnoDB;
...
CREATE TABLE analytics.gh_events (
    ...
    action VARCHAR(100), occurred_at DATETIME
) ENGINE=InnoDB;

CREATE TABLE analytics.gh_stars (
    user_login VARCHAR(255) PRIMARY KEY, user_id INT, starred_at DATETIME
) ENGINE=InnoDB;
...
## Verify
```sql
SELECT is_closed FROM analytics.gh_issues LIMIT 5;           -- 0 or 1
SELECT typeof(committed_at) FROM analytics.gh_commits LIMIT 1;
SELECT issue_id, COUNT(*) cnt FROM analytics.gh_issues GROUP BY issue_id HAVING cnt>1;
```
